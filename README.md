<img width="1470" height="923" alt="image" src="https://github.com/user-attachments/assets/a48ca23d-0866-49e3-99ae-4401873b7dfb" /># GlassReader

A lightweight native macOS reader designed for local reading.

GlassReader is a free-to-use native macOS application for reading comics, PDF documents, images, and compressed archives. It provides a clean interface for local reading.
<img width="1470" height="923" alt="image" src="https://github.com/user-attachments/assets/0b53198f-2ea8-48d3-aa46-243c6a655e5e" />

---

## Features

- Open local folders, files, and supported archives
- Drag and drop files directly into the reader
- Read ZIP / CBZ / RAR / 7Z archives
- Password-protected archive support
- PDF document support
- JPG / JPEG / PNG / WebP / GIF / BMP / TIFF / HEIC image support
- Single-page and double-page reading modes
- Normal left-to-right and manga right-to-left reading modes
- Five image display modes:
  - Fit Screen
  - Fit Width
  - Original Size
  - Fill Screen
  - Smart Fit
- Multi-tab reading, with tabs restored after relaunch
- Per-file reading progress memory
- Per-file single/double-page and reading-direction memory
- Reading history and favorites
- Favorite toggle with a star button
- Private browsing mode and clear-history action
- Thumbnail preview and page navigation
- Mouse click, keyboard, and mouse-wheel page navigation
- Slideshow / auto-play reading
- Standard fullscreen and immersive fullscreen modes
- Floating fullscreen toolbar and progress bar overlays
- Keyboard shortcut panel with clickable actions
- Chinese, English, Japanese, Korean, Russian, French, German, and Spanish UI
- macOS “Open With” integration for supported document types
- Native SwiftUI/AppKit interface
- Designed for local reading without an account

## Requirements

- macOS 14.0 or later
- Apple Silicon Mac

## Privacy

GlassReader is intended for local reading. Favorites, reading history, page progress, and reading preferences are stored locally on the Mac. Archive passwords are kept for the current app session so an already-open archive can be revisited without asking again; they are not intended to be uploaded to a server.

## Download

Download the latest release from the **Releases** page.

## Contributing

Issues and pull requests are welcome. Please include the macOS version, GlassReader version, file format, and a short reproduction description when reporting a problem.

## License

Copyright (C) 2026 Chen Yifu (ifuchen113)

GlassReader is free software distributed under the **GNU General Public License, Version 3.0 only**.

See the [`LICENSE`](LICENSE) file for the license notice and the official GPLv3 reference.

---

# GlassReader阅读器

一款专注于本地阅读体验的轻量级原生 macOS 阅读器。

GlassReader 是一款免费使用的 macOS 阅读器，可用于阅读漫画、PDF 文档、图片以及压缩包文件，提供简洁的本地阅读界面。
<img width="1470" height="923" alt="image" src="https://github.com/user-attachments/assets/8f65660b-3e06-498d-87f2-7f311a368052" />

---

## 功能特点

- 支持打开本地文件夹、文件和压缩包
- 支持直接拖放文件到阅读器中打开
- 支持 ZIP / CBZ / RAR / 7Z 压缩包
- 支持带密码的压缩包
- 支持 PDF 文档
- 支持 JPG / JPEG / PNG / WebP / GIF / BMP / TIFF / HEIC 图片
- 支持单页 / 双页阅读模式
- 支持普通左到右和日漫右到左阅读模式
- 提供五种图片初始显示模式：
  - 适应屏幕
  - 适应宽度
  - 原始大小
  - 填满屏幕
  - 智能适应
- 支持多标签页阅读，退出后可以恢复已打开的标签页
- 记忆每个文件的上次阅读进度
- 记忆每个文件的单页 / 双页和普通 / 日漫阅读设置
- 支持收藏夹和历史记录
- 支持通过星星按钮快速收藏和取消收藏
- 支持无痕浏览和一键清空历史记录
- 支持缩略图预览和点击跳页
- 支持鼠标单击、键盘和鼠标滚轮翻页
- 支持自动播放幻灯片
- 支持普通全屏和纯净全屏
- 纯净全屏支持浮动顶部状态栏和底部进度条
- 提供快捷键列表，列表中的功能可以直接点击使用
- 支持中文、English、日本語、한국어、Русский、Français、Deutsch、Español
- 支持在 macOS 的“打开方式”中选择 GlassReader
- 使用原生 SwiftUI / AppKit 构建
- 面向本地阅读，不需要账号

## 系统要求

- macOS 14.0 或更高版本
- Apple Silicon Mac

## 隐私说明

GlassReader 专注于本地阅读。收藏夹、历史记录、阅读进度和阅读设置会保存在 Mac 本地。为了让已经打开的压缩包可以在标签页之间快速切换，压缩包密码会在当前 App 会话中暂存，不会上传到服务器。

## 下载

请前往 **Releases** 页面下载最新版本。

## 参与贡献

欢迎提交 Issue 和 Pull Request。反馈问题时，建议附上 macOS 版本、GlassReader 版本、文件格式以及可以复现问题的简短步骤。

## 许可证

版权所有 © 2026 陈毅夫（GitHub：ifuchen113）

GlassReader 采用 **GNU General Public License, Version 3.0 only（GPL-3.0-only）** 开源。

许可说明请查看 [`LICENSE`](LICENSE) 文件。
